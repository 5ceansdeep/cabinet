...
- **바이럴용 인증물 (요청)** — 추천 순서 1 → 2 → 3
  1. **보관증 카드**: 서랍에 넣고 나면 종이 보관증 한 장. 보관번호(No. 0768)·날짜·서랍 이름·곡 목록·그때 쓴 편지 한 줄,
     붉은 도장, 우하단에 작게 `CABINET · cabinet.app` 워터마크. 이미지로 저장·공유.
     5.1 보고서 디자인을 그대로 써서 새로 만들 게 적다
  2. **공개 서랍 링크 `/shelf/{id}`**: 읽기 전용 페이지. 링크 미리보기(OG 이미지)에 네임택 + 앨범 커버 4장 모자이크.
     트위터·디스코드·카톡에서 그림이 바로 보이니 확산에 제일 유리하다
  3. **3~5초 클립 저장**: 서랍이 닫히고 네임택에 타자기로 이름이 찍히는 장면을 MediaRecorder 로 녹화해 내려받기.
     움직이는 결과물이라 SNS 에서 제일 잘 퍼지지만 구현 난이도가 가장 높다
  4. **같은 편지, 다른 결과 비교**: "이 편지로 나는 이런 곡을 받았다" 링크 → 받은 사람이 같은 편지로 자기 결과를 뽑아 비교.
     챌린지 성격
