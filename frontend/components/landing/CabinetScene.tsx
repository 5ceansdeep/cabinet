...
/* 카메라는 고정. dive 때만 맨 위 서랍 속으로 빨려 들어간다 */
...
/* 1·2번 — 순백의 공간 저 멀리 서 있는 서류함. 맨 위 서랍이 톡톡 들썩이며 부르고, 호버하면 쫙 펼쳐지고,
...
export default function CabinetScene({ fields, phase, onDone }: { fields: Field[]; phase: Phase; onDone: (values: Record<string, string>) => void }) {
  ...
  const [sealed, setSealed] = useState(false); // 다 받고 서랍을 "탁" 닫았는지
  ...
  function submit(e: FormEvent<HTMLFormElement>, name: string) {
    ...
    // 마지막 파일이 제자리로 들어가면 서랍을 닫는다
    setTimeout(() => {
      setSealed(true);
      thud(80);
    }, 700);
    setTimeout(() => onDone(values.current), 1300);
  }

  const slide = phase === "dive" ? FULL_OPEN : open && !sealed ? FULL_OPEN : 0;
  const inputCls =
